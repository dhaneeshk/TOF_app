...
_declspec (dllexport) long CC Initialize_Ethernet_DG_BME(long BusNumber, long base, char* p_ip_address, long Product, long DG_Number)
{
	...
	result = LoadCommandRegister(&(p_Present->CommandRegister), p_Present);
	if (result != DgSuccess)
		return result;
	...
	result = Synchronize_DG_State(DG_Number);
	if (result != DgSuccess)
		return result;
	switch (p_Present->DG_Product)
	{
...
_declspec (dllexport) long CC IgnoreCommands(BOOL Ignore)
{
	if (Ignore)
		BmeDelgenSetIgnore(1);
	else
		BmeDelgenSetIgnore(0);
	return DgSuccess;
}
...
#define CurrentParameterFileVersion					6
														 ...
														 BOOL* p_CalibrateTiming, unsigned long* p_CalibrationLevel, char* FileName)
{
	...
	if (stream != NULL)
	{
		...
		case 5:
				...
				p_DGS_V5 = malloc(NoDelayGenerators * sizeof(DG_BME_State_V5));
				fseek(stream, position, 0);
				fread(p_DGS_V5, sizeof(DG_BME_State_V5), NoDelayGenerators, stream);
				if (!feof(stream))
				{
					fread(&(TestChar), sizeof(char), 1, stream);
					if (feof(stream))
					{
						CopyState_V5(p_DGS, p_DGS_V5);
						break;
					}
				}
				fseek(stream, position, 0);
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						p_Present->OscillatorIncrement = 1.0 / (p_Present->ExternalOscillatorFrequency * max(1.0, p_ST->TriggerMultiplier));
					}
				}
			}
		...
		while (DG_Number < NoDelayGenerators)
		{
			...
			switch (p_Present->DG_Product)
			{
				...
				case BME_SG08P2:
					...
					break;
				case BME_SG08P3:
					fprintf(stream, "BME_SG08P3    ");
					break;
				case BME_SG08P4:
					fprintf(stream, "BME_SG08P4    ");
					break;
				default:
					break;
			}
...
_declspec (dllexport) long CC CalibrateGroupDirect(int* p_module, double* p_internal, double* p_external)
{
	DG_InternalData* p_Present;
	DG_InternalData* p_MasterCard;
	long DG_Number;
	...
	while (DG_Number < NoDelayGenerators)
	{
		...
		switch (p_Present->DG_Product)
		{
			...
			case BME_SG08P3:
			case BME_SG08P4:
				p_Present->InternalOscillatorFrequency = *(p_internal + DG_Number);
				p_Present->ExternalOscillatorFrequency = *(p_external + DG_Number);
				if (*(p_module + DG_Number) == MasterModule)
				{
					p_MasterCard = p_Present;
					p_Present->p_Master = p_MasterCard;
...
_declspec (dllexport) long CC Set_DG_BME(DG_BME_Registers* p_pdg, long DG_Number)
{
	long result;
	DG_InternalData* p_Present;
	unsigned long cntrl,DG_Product;
	...
	case BME_SG08P4:
		...
		{
			result = Set_Prescaler(p_pdg, p_LoadWord, p_Present);
			if (result != DgSuccess)
				return result;
		}
		break;
	...
	case BME_SG08P4:
		...
		{
			result = Set_Delay_Trigger(&(p_pdg->DT), p_LoadWord, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->A), &(p_Present->DelayGeneratorState.A), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->A), &(p_Present->DelayGeneratorState.A), p_LoadWord, 0, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->B), &(p_Present->DelayGeneratorState.B), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->B), &(p_Present->DelayGeneratorState.B), p_LoadWord, 1, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->C), &(p_Present->DelayGeneratorState.C), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->C), &(p_Present->DelayGeneratorState.C), p_LoadWord, 2, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->D), &(p_Present->DelayGeneratorState.D), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->D), &(p_Present->DelayGeneratorState.D), p_LoadWord, 3, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->E), &(p_Present->DelayGeneratorState.E), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->E), &(p_Present->DelayGeneratorState.E), p_LoadWord, 4, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->F), &(p_Present->DelayGeneratorState.F), DG_Product))
		{
			result = Set_Delay_channel_SG05P4(&(p_pdg->F), &(p_Present->DelayGeneratorState.F), p_LoadWord, 5, p_Present);
			if (result != DgSuccess)
				return result;
		}
		break;
	...
	case BME_SG05P3:
		...
		{
			result = Set_Main_Counter(&(p_pdg->MC), p_Present);
			if (result != DgSuccess)
				return result;
		}
	case BME_G03V3:
	...
	default:
		...
		{
			result = Set_Reference_channel(&(p_pdg->T0), p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->A), &(p_Present->DelayGeneratorState.A), DG_Product))
		{
			result = Set_Delay_channel(&(p_pdg->A), &(p_Present->DelayGeneratorState.A), 1, p_Present);
			if (result != DgSuccess)
				return result;
		}
		if (!IsEqualDC(&(p_pdg->B), &(p_Present->DelayGeneratorState.B), DG_Product))
		{
			result = Set_Delay_channel(&(p_pdg->B), &(p_Present->DelayGeneratorState.B), 2, p_Present);
			if (result != DgSuccess)
				return result;
		}
	}
	...
	else
	{
		...
		case BME_SG08P3:
		case BME_SG08P4:
			cntrl = (p_Present->CommandRegister & 0xFFFF0000) | p_Present->CommandRegisterState;
			cntrl &= (p_Present->CommandRegister | ~0xFFFF0000);
			result = LoadCommandRegister(&cntrl, p_Present);
			if (result != DgSuccess)
				return result;
		default:
			break;
	...
	if (*p_LoadWord != 0x0)
	{
		...
		case BME_SG08P1:
			*p_LoadWord &= ~0x18;
			result = Data32Out(p_Present->InitializeAddress, p_LoadWord, &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			*p_LoadWord = 0x0;
			break;
...
_declspec (dllexport) long CC SetDelayGenerator(DG_BME_Control* p_BDG, long DG_Number)
{
	...
	if ((dgr.OutputLevel < TTL_VoltageLevel) || (ECL_VoltageLevel < dgr.OutputLevel))
		return InvalidOutputLevel;
	dgr.PulseWidth = p_BDG->PulseWidth;

	result = SetTrigger(&dgr, p_BDG, DG_Number);
	if (result != 0)
		return result;
	if (((p_BDG->DT.MS_Bus & EnableFromE) != 0x0) && (!p_BDG->E.Disconnect))
		return OutputMustBeDisconnected;
	if ((p_BDG->DT.UseF) && (!p_BDG->F.Disconnect))
		return OutputMustBeDisconnected;
	...
	default:
		...
		result = SetReference(&(dgr.T0), &(p_BDG->T0), dgr.MC.Digital, p_BDG->OutputLevel, DG_Number);
		if (result != 0)
			return result;
		p_Present->TimeIncrement = p_Present->A_Increment;
		result = SetChannel(&(dgr.A), &(p_BDG->A), dgr.MC.Digital, p_BDG->OutputLevel, DG_Number);
		if (result != 0)
			return result;
		p_Present->TimeIncrement = p_Present->B_Increment;
		result = SetChannel(&(dgr.B), &(p_BDG->B), dgr.MC.Digital, p_BDG->OutputLevel, DG_Number);
		if (result != 0)
			return result;
		break;
	}
													...
													long DG_Number)
{
	...
	BOOL b_ModuloCounter;
	InternalArm = FALSE;
	SoftwareTrigger = FALSE;
	b_ModuloCounter = FALSE;
	if ((DG_Number < 0) || (NoDelayGenerators <= DG_Number))
		return DgNumberOutOfRange;
	p_Present = (p_Internal + DG_Number);
	if (p_Present->b_DG_Activated)
	...
	result = LoadCardParameters(FALSE, b_ModuloCounter, FALSE, DG_Number);
	if (result != 0)
		return result;
	result = Activate_DG_BME(DG_Number);
	if (result != 0)
													...
													long DG_Number)
{
	...
	if (p_Present->b_DG_Activated)
	{
		Action = Modify_DG(0, Active_T0, Positive_T0, Terminate_T0, HighDrive_T0,
													...
													long DG_Number)
{
	...
	case BME_SG08P3:
	case BME_SG08P4:
		if (InternalClock > 0.0)
			dgc.DT.ListLength = max(1, dgc.DT.ListLength);
		else
			dgc.DT.ListLength = max(0, dgc.DT.ListLength);
		break;
	default:
		break;
	}
...
_declspec (dllexport) long CC Set_ResetWhenDone(BOOL ResetWhenDone, long DG_Number)
{
	DG_BME_Control dgc;
	long result;
	DG_InternalData* p_Present;
...
_declspec (dllexport) long CC Set_G08_UnsynchParameters(DG_BME_Control* p_BDG, long DG_Number)
{
	DG_BME_Control dgc;
	long result;
	DG_InternalData* p_Present;
	...
	p_Present = (p_Internal + DG_Number);
	memcpy(&dgc, &(p_Present->DelayControlState), sizeof(DG_BME_Control));
	dgc.A.GoSignal |= (SynchReload & p_BDG->A.GoSignal);
	dgc.A.GoSignal &= (~SynchReload | p_BDG->A.GoSignal);
	dgc.B.GoSignal |= (SynchReload & p_BDG->B.GoSignal);
	dgc.B.GoSignal &= (~SynchReload | p_BDG->B.GoSignal);
	dgc.C.GoSignal |= (SynchReload & p_BDG->C.GoSignal);
	dgc.C.GoSignal &= (~SynchReload | p_BDG->C.GoSignal);
	dgc.D.GoSignal |= (SynchReload & p_BDG->D.GoSignal);
	dgc.D.GoSignal &= (~SynchReload | p_BDG->D.GoSignal);
	dgc.E.GoSignal |= (SynchReload & p_BDG->E.GoSignal);
	dgc.E.GoSignal &= (~SynchReload | p_BDG->E.GoSignal);
	dgc.F.GoSignal |= (SynchReload & p_BDG->F.GoSignal);
	dgc.F.GoSignal &= (~SynchReload | p_BDG->F.GoSignal);
	dgc.DT.MS_Bus |= (GateBurstSynch & p_BDG->DT.MS_Bus);
	dgc.DT.MS_Bus &= (~GateBurstSynch | p_BDG->DT.MS_Bus);
	dgc.DT.MS_Bus |= (ExtSynchFromF & p_BDG->DT.MS_Bus);
	dgc.DT.MS_Bus &= (~ExtSynchFromF | p_BDG->DT.MS_Bus);
	dgc.DT.ClockSource = p_BDG->DT.ClockSource;
	dgc.DT.TriggerMultiplier = p_BDG->DT.TriggerMultiplier;
	dgc.DT.TriggerLevel = p_BDG->DT.TriggerLevel;
	dgc.DT.GateLevel = p_BDG->DT.GateLevel;
	result = SetDelayGenerator(&dgc, DG_Number);
	if (result != 0)
		return result;
	return DgSuccess;
...
_declspec (dllexport) long CC Set_InhibitList(double* p_InhibitTime, unsigned long* p_StepBack, signed long ListLength, 
																														unsigned long TriggerSignal, long DG_Number)
{
	DG_BME_Control dgc;
	DelayTriggerData* p_DT;
	long result;
	...
	p_DT->ListLength = min(ListLength, MemLength);
	for (k1 = 0; k1 < p_DT->ListLength; k1++)
	{
		p_DT->InhibitTime[k1] = *(p_InhibitTime + k1);
		p_DT->StepBack[k1] = *(p_StepBack + k1);
	}
	p_DT->TriggerSignal = TriggerSignal;
															...
															BOOL Disconnect, BOOL OntoMsBus, BOOL InputPositive, long DG_Number)
{
	...
	p_ch->StepBack[0] = 0;
	if ((p_ch->GoSignal & 0x77) != 0x0)
	{
		p_ch->FireFirst = FireFirst;
		p_ch->PulseWidth[0] = PulseWidth;
		p_ch->ListLength = max(1, p_ch->ListLength);
	}
	else
	{
		p_ch->FireFirst = -1.0;
		p_ch->PulseWidth[0] = -1.0;
		p_ch->ListLength = 0;
	}
...
_declspec (dllexport) long CC CopyState_V5(DG_BME_State* p_DGS, DG_BME_State_V5* p_DGS_V5)
{
	ResetState(p_DGS);
	p_DGS->BaseAddress = p_DGS_V5->BaseAddress;
	p_DGS->Master = p_DGS_V5->Master;
	p_DGS->DG_Product = p_DGS_V5->DG_Product;
	p_DGS->BusNumber = p_DGS_V5->BusNumber;
	CopyControl_V5(&(p_DGS->Control), &(p_DGS_V5->Control));
	return DgSuccess;
}
...
_declspec (dllexport) long CC CopyControl_V5(DG_BME_Control* p_Control, DG_BME_Control_V5* p_Control_V5)
{
	ResetControl(p_Control);
	CopyTriggerControl_V5(&(p_Control->DT), &(p_Control_V5->DT));
	memcpy(&(p_Control->T0), &(p_Control_V5->T0), sizeof(DelayChannelData));
	memcpy(&(p_Control->A), &(p_Control_V5->A), sizeof(DelayChannelData));
	memcpy(&(p_Control->B), &(p_Control_V5->B), sizeof(DelayChannelData));
	memcpy(&(p_Control->C), &(p_Control_V5->C), sizeof(DelayChannelData));
	memcpy(&(p_Control->D), &(p_Control_V5->D), sizeof(DelayChannelData));
	memcpy(&(p_Control->E), &(p_Control_V5->E), sizeof(DelayChannelData));
	memcpy(&(p_Control->F), &(p_Control_V5->F), sizeof(DelayChannelData));
	p_Control->Gate_AB = p_Control_V5->Gate_AB;
	p_Control->GateFunction = p_Control_V5->GateFunction;
	p_Control->OutputLevel = p_Control_V5->OutputLevel;
	p_Control->DG_Product = p_Control_V5->DG_Product;
	p_Control->PulseWidth = p_Control_V5->PulseWidth;
	return DgSuccess;
}
...
_declspec (dllexport) long CC Deactivate_DG_BME(long DG_Number)
{
	...
	case BME_SG08P3:
	case BME_SG08P4:
		cntrl = p_Present->CommandRegister & 0x3F3FFE7B;
		break;
	default:
...
_declspec (dllexport) long CC Activate_DG_BME(long DG_Number)
{
	...
	case BME_SG08P3:
	case BME_SG08P4:
		result = LoadCounterControl(p_Present->CounterControlRegister, p_Present);
		if (result != DgSuccess)
			return result;
	default:
		break;
...
_declspec (dllexport) long CC ResetEventCounter(long DG_Number)
{
	...
	p_Present = (p_Internal + DG_Number);
	Value = 0x01;
	result = DataOut(p_Present->InitializeAddress, &Value, &(p_Present->PciDelgen), -1);
	if (result != DgSuccess)
		return result;
	return DgSuccess;
}
...
{
	long result;
	long DG_Number = NoDelayGenerators - 1;
	while (DG_Number >= 0)
	{
		result = LoadCardParameters(b_EventCounter, b_ModuloCounter, b_TimeList, DG_Number);
		if (result != DgSuccess)
			return result;
		DG_Number--;
	}
...
_declspec (dllexport) long CC ResetCounterAllCards(BOOL b_EventCounter, BOOL b_ModuloCounter, BOOL b_TimeList)
{
	...
	for (DG_Number = 0; DG_Number < NoDelayGenerators; DG_Number++)
	{
		result = ResetCounter(b_EventCounter, b_ModuloCounter, b_TimeList, DG_Number);
		if (result != DgSuccess)
			return result;
	}
	return DgSuccess;
...
_declspec (dllexport) long CC Read64BitTriggerCounter(_int64* p_result, long DG_Number)
{
	...
	*p_result = 0x0;
	...
	{
		case BME_G02V1:
			ErrorCode = DataIn(p_Present->CommandAddress, &result, &(p_Present->PciDelgen), -1);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			if ((result & 0x10) != 0)
			{
				*p_result = 0xFFFFFFFF;
			...
			ErrorCode = DataIn(p_Present->ModuloCounterAddress, p_result, &(p_Present->PciDelgen), 4);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			break;
		case BME_G02V2:
		...
		case BME_SG05P2:
		case BME_SG05P3:
			ErrorCode = DataIn(p_Present->TriggerCounterAddress, p_result, &(p_Present->PciDelgen), 4);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			break;
		case BME_SG08P1:
		...
		case BME_SG08P3:
		case BME_SG08P4:
			ErrorCode = Data32In(p_Present->TriggerCounterAddress, p_result, &(p_Present->PciDelgen), 8);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			break;
		default:
...
_declspec (dllexport) long CC ReadModuleCalibration(double* p_InternalOscillatorFrequency, double* p_ExternalOscillatorFrequency, 
																		double* p_TenMhzp2Time, double* p_TenMhzp1Time, double* p_TenMhzZeroTime,
																		unsigned long* p_DG_Product, int* p_MS_Module, long DG_Number)
{
	DG_InternalData* p_Present;
	if ((DG_Number < 0) || (NoDelayGenerators <= DG_Number))
		return DgNumberOutOfRange;
	p_Present = (p_Internal + DG_Number);
	*p_DG_Product = p_Present->DG_Product;
	if (p_Present == p_Present->p_Master)
		*p_MS_Module = MasterModule;
	else
		*p_MS_Module = SlaveModule;
	*p_InternalOscillatorFrequency = p_Present->InternalOscillatorFrequency;
	*p_ExternalOscillatorFrequency = p_Present->ExternalOscillatorFrequency;
	*p_TenMhzp2Time = p_Present->TenMhzp2Time;
	*p_TenMhzp1Time = p_Present->TenMhzp1Time;
	*p_TenMhzZeroTime = p_Present->TenMhzZeroTime;
	return DgSuccess;
}
...
_declspec (dllexport) long CC ReadFrequencyCounter(double* p_Frequency, long DG_Number)
{
	...
	switch (DG_Product)
	{
		...
		case BME_SG08P3:
		case BME_SG08P4:
			ErrorCode = Data32In(p_Present->FrequencyCounterAddress, p_result, &(p_Present->PciDelgen), 4);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
...
_declspec (dllexport) long CC EnableInterrupts(unsigned long value, long DG_Number)
{
	...
	if ((DG_Number < 0) || (NoDelayGenerators <= DG_Number))
		return DgNumberOutOfRange;
	p_Present = (p_Internal + DG_Number);
	ErrorCode = DataOut(p_Present->InterruptEnableAddress, &IntCntrl, &(p_Present->PciDelgen), -1);
	if (ErrorCode != DgSuccess)
		return ErrorCode;
	return DgSuccess;
}
...
_declspec (dllexport) long CC AcknowledgeInterrupts(unsigned long value, long DG_Number)
{
	...
	switch (DG_Product)
	{
		...
		case BME_SG08P3:
		case BME_SG08P4:
			ErrorCode = Data32Out(p_Present->InterruptAcknowledgeAddress, &value, &(p_Present->PciDelgen), 4);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			break;
		default:
			ErrorCode = DataOut(p_Present->InterruptAcknowledgeAddress, &IntCntrl, &(p_Present->PciDelgen), -1);
			if (ErrorCode != DgSuccess)
				return ErrorCode;
			break;
	}
...
_declspec (dllexport) long CC LoadOutputState(long DG_Number)
{
	...
	{
		cntrl |= 0x80;
		ErrorCode = Data32Out(p_Present->CommandAddress, &cntrl, &(p_Present->PciDelgen), -1);
		if (ErrorCode != DgSuccess)
			return ErrorCode;
		cntrl &= 0x3F3E7F;
		ErrorCode = Data32Out(p_Present->CommandAddress, &cntrl, &(p_Present->PciDelgen), -1);
...
_declspec (dllexport) unsigned long CC ModifyAction(DG_BME_Control* p_BDG, long DG_Number)
{
	...
	p_Present = (p_Internal + DG_Number);
	DG_Product = p_Present->DG_Product;
	if (DG_Product != p_BDG->DG_Product)
		Action |= 0x81;
  ...
  //Device.SerialNumber[0] = '\0';
	...
	for (i1 = 0; i1 < NoDelgen; i1++)
	{
		if (DelgenList[i1].Master == i1)
		{
			...
			p_IpDelgen->Master = Count + NumberPciDelayGenerators;
			for (LineNo = 0; LineNo < 8; LineNo++)
				p_IpDelgen->LineMaster[LineNo] = Count + NumberPciDelayGenerators;
			...
			while (found)
			{
				...
				if (found)
				{
					...
					p_IpDelgen->Master = MasterCount + NumberPciDelayGenerators;
					p_IpDelgen->Selected = TRUE;
					for (LineNo = 0; LineNo < 8; LineNo++)
						p_IpDelgen->LineMaster[LineNo] = MasterCount + NumberPciDelayGenerators;
			...
			for (k1 = 0; k1 < NoDelgen; k1++)
			{
				for (LineNo = 0; LineNo < 8; LineNo++)
				{
					if (MasterLineDG((p_IpDelgen + k1), (p_IpDelgen + i1), LineNo))
						(p_IpDelgen + i1)->LineMaster[LineNo] = k1 + NumberPciDelayGenerators;
					else
...
long Synchronize_DG_State(long DG_Number)
{
	DG_BME_Registers dgr;
	DG_InternalData* p_Present;
	unsigned long DG_Product;
...
//		}
		...
		result = LoadCounterControl(CounterControlValue, p_Present);
		if (result != DgSuccess)
			return result;
		...
		case BME_SG08P1:
			Value = 0x1F003F1F;
			result = Data32Out(p_Present->InitializeAddress, &Value, &(p_Present->PciDelgen), -1);
			break;
		case BME_SG08P2:
		case BME_SG08P3:
		case BME_SG08P4:
		...
		if (result != DgSuccess)
			return result;
		Value = 0xFFFF;
		result = DataOut(p_Present->InterruptAcknowledgeAddress, &Value, &(p_Present->PciDelgen), -1);
		if (result != DgSuccess)
			return result;
		Value = 0x0;
		result = DataOut(p_Present->InterruptEnableAddress, &Value, &(p_Present->PciDelgen), -1);
		if (result != DgSuccess)
			return result;
		Value = 0xFFFF;
		result = DataOut(p_Present->InterruptAcknowledgeAddress, &Value, &(p_Present->PciDelgen), -1);
	...
	case BME_G03P2:
		...
		p_Present->PrescalerState[0] = 0xFFFFFFFF;
		p_Present->PrescalerState[1] = 0xFFFFFFFF;
		result = LoadPrescaler(PrescalerValue, p_Present);
		if (result != DgSuccess)
			return result;
...
long LoadPrescaler(unsigned long* p_value, DG_InternalData* p_Present)
{
	long result;
	int NumBytes;
	result = DgSuccess;
						...
						(*(p_value + 2) != p_Present->PrescalerState[2]))
	{
		...
		case BME_SG08P4:
			NumBytes = 12;
			result = Data32Out(p_Present->PrescalerAddress, &(p_Present->PrescalerState), 
		...
		case BME_SG05P3:
			NumBytes = 8;
			result = DataOut(p_Present->PrescalerAddress, &(p_Present->PrescalerState), 
																							&(p_Present->PciDelgen), NumBytes);
			break;
		case BME_G03V3:
			NumBytes = 2;
...
long LoadCounterControl(unsigned long* p_value, DG_InternalData* p_Present)
{
	int NumBytes,i1;
	long result;
	BOOL different;
	...
	case BME_SG08P4:
		...
		if (different)
		{
			...
			//AfxMessageBox(info, MB_OK, 0);	
			NumBytes = 20;
			result = Data32Out(p_Present->CounterControlAddress, p_Present->CounterControlState,
																						&(p_Present->PciDelgen), NumBytes);
		}
		break;
	case BME_G05P2:
	...
	case BME_SG05P3:
							...
							(*(p_value + 1) != p_Present->CounterControlState[1]))
		{
			...
			NumBytes = 8;
			result = DataOut(p_Present->CounterControlAddress, p_Present->CounterControlState,
																						&(p_Present->PciDelgen), NumBytes);
		}
		break;
	default:
...
long LoadCommandRegister(unsigned long* p_value, DG_InternalData* p_Present)
{
	long result;
	result = DgSuccess;
	if (*p_value != p_Present->CommandRegisterState)
	{
		...
		case BME_SG08P3:
		case BME_SG08P4:
			result = Data32Out(p_Present->CommandAddress, p_value, &(p_Present->PciDelgen), -1);
			break;
		default:
			result = DataOut(p_Present->CommandAddress, p_value, &(p_Present->PciDelgen), -1);
			break;
		}
					...
					DelayChannel* p_k, DelayChannel* p_ChState, short int pos, DG_InternalData* p_Present)
{
	...
	p_ChState->GoSignal |= (p_k->GoSignal & 0x77E77);
	p_ChState->GoSignal &= (p_k->GoSignal | ~0x77E77);
}
																				...
																				DelayTriggerType* p_dt, DG_InternalData* p_Present)
{
	...
	case BME_SG08P3:
	case BME_SG08P4:
		if (p_dt->ListLength > 1)
		{
			if ((p_dt->TriggerSignal & RunCircle) != 0x0)
				*p_FinalValue |= 0x1000000;
			else
				*p_FinalValue &= ~0x1000000;
			if ((p_dt->TriggerSignal & StartBus) != 0x0)
				*p_FinalValue |= 0x2000000;
			else
				*p_FinalValue &= ~0x2000000;
			if ((p_dt->TriggerSignal & StartLocal) != 0x0)
				*p_FinalValue |= 0x4000000;
			else
				*p_FinalValue &= ~0x4000000;
			if ((p_dt->TriggerSignal & StepBackBus) != 0x0)
				*p_FinalValue |= 0x8000000;
			else
				*p_FinalValue &= ~0x8000000;
			if ((p_dt->TriggerSignal & StepBackLocal) != 0x0)
				*p_FinalValue |= 0x10000000;
			else
				*p_FinalValue &= ~0x10000000;
		}
		else
			*p_FinalValue &= ~0x1F000000;
	case BME_SG08P1:
	case BME_SG08P2:
		...
		{
			MinCount = min((int)(p_dt->BurstCounter + 1), (int)p_dt->ListLength);
			MaxInhibit = (int)p_dt->InhibitTime[0];
			for (i1 = 1; i1 < MinCount; i1++)
				MaxInhibit = max(MaxInhibit, (int)(p_dt->InhibitTime[i1]));
			*p_FinalValue |= 0x01;
			if ((int)p_dt->ForceTrigger > (MaxInhibit + 4))
				*p_FinalValue |= 0x04;
			else
				*p_FinalValue &= ~0x04;
		}
...
long Set_Main_Counter(MainCounterType* p_mc, DG_InternalData* p_Present)
{
	long result;
	unsigned long DG_Product;
	if (p_mc->Active)
	{
								...
								(p_mc->Int3 != p_Present->DelayGeneratorState.MC.Int3))
		{
			...
			case BME_SG02V4:
				break;
			default:
				result = DataOut(p_Present->Delay_Aux, &(p_mc->Digital), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
				if ((DG_Product == BME_G03V1) || (DG_Product == BME_G03V2))
				{
					result = DataOut(p_Present->Delay_Aux, &(p_mc->Int3), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
				}
			}
...
long Set_Delay_Trigger(DelayTriggerType* p_dt, unsigned long* p_LoadWord, DG_InternalData* p_Present)
{
	long result;
	int i1;
	BOOL same;
	unsigned long LowWord, HighWord, ShiftWord;
	unsigned long SerialWord, LargeInhibit;
	...
	case BME_SG08P4:
		...
		if ((p_dt->TriggerDAC & 0x0FFF) != (p_Present->DelayGeneratorState.DT.TriggerDAC & 0x0FFF))
		{
			...
			result = Data32Out(p_Present->Serial_DAC, &SerialWord, &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
		}
		...
		if (p_dt->StopOnPreset)
		{
			if (p_dt->PresetLong != p_Present->DelayGeneratorState.DT.PresetLong) 
			{
				...
				result = Data32Out(p_Present->TriggerCounterAddress, &(p_dt->PresetLong), &(p_Present->PciDelgen), 8);
				if (result != DgSuccess)
					return result;
				*p_LoadWord |= 0x8;
			}
	...
	case BME_SG05P3:
		if ((p_dt->TriggerDAC & 0x0FFF) != (p_Present->DelayGeneratorState.DT.TriggerDAC & 0x0FFF))
		{
			...
			result = DataOut(p_Present->DAC, &(p_dt->TriggerDAC), &(p_Present->PciDelgen), 2);
			if (result != DgSuccess)
				return result;
		}
	case BME_G02V1:
	...
	case BME_G03V2:
		if (p_dt->StopOnPreset)
		{
			if (p_dt->PresetValue != p_Present->DelayGeneratorState.DT.PresetValue) 
			{
				...
				result = DataOut(p_Present->TriggerCounterAddress, &(p_dt->PresetValue), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
	...
	case BME_SG05P3:
								...
								(p_dt->ForceTrigger != p_Present->DelayGeneratorState.DT.ForceTrigger))
		{
			...
			result = DataOut(p_Present->ModuloCounterAddress, &(p_dt->InhibitTrigger), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			p_Present->DelayGeneratorState.DT.ForceTrigger = p_dt->ForceTrigger;
			result = DataOut(p_Present->ModuloCounterAddress, &(p_dt->ForceTrigger), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
		}
		break;
	...
	case BME_SG08P2:
								...
								(p_dt->BurstCounter != p_Present->DelayGeneratorState.DT.BurstCounter))
		{
			...
			//AfxMessageBox(info, MB_OK, 0);	
			p_Present->DelayGeneratorState.DT.InhibitTrigger = p_dt->InhibitTrigger;
			result = Data32Out(p_Present->ModuloCounterAddress, &(p_dt->InhibitTrigger), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			p_Present->DelayGeneratorState.DT.ForceTrigger = p_dt->ForceTrigger;
			result = Data32Out(p_Present->ModuloCounterAddress, &(p_dt->ForceTrigger), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			p_Present->DelayGeneratorState.DT.StepBackCounter = p_dt->StepBackCounter;
			result = Data32Out(p_Present->ModuloCounterAddress, &(p_dt->StepBackCounter), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			p_Present->DelayGeneratorState.DT.BurstCounter = p_dt->BurstCounter;
			result = Data32Out(p_Present->ModuloCounterAddress, &(p_dt->BurstCounter), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			*p_LoadWord |= 0x8;
		}
								...
								(p_dt->DelaySecondary != p_Present->DelayGeneratorState.DT.DelaySecondary))
		{
			...
			result = Data32Out(p_Present->SecondaryCounterAddress, &(p_dt->InhibitSecondary), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			p_Present->DelayGeneratorState.DT.DelaySecondary = p_dt->DelaySecondary;
			result = Data32Out(p_Present->SecondaryCounterAddress, &(p_dt->DelaySecondary), &(p_Present->PciDelgen), 4);
			if (result != DgSuccess)
				return result;
			*p_LoadWord |= 0x8;
		}
	...
	case BME_SG08P3:
	case BME_SG08P4:
		if (p_dt->ListLength > 0)
		{
			same = TRUE;
			if (p_dt->ListLength != p_Present->DelayGeneratorState.DT.ListLength)
				same = FALSE;
			i1 = 0;
			while ((same) && (i1 < p_dt->ListLength))
			{
				if (p_dt->InhibitTime[i1] != p_Present->DelayGeneratorState.DT.InhibitTime[i1])
					same = FALSE;
				if (p_dt->StepBack[i1] != p_Present->DelayGeneratorState.DT.StepBack[i1])
					same = FALSE;
				i1++;
			}
			if(!same)
			{
				IniChar = 0x20000000;
				result = Data32Out(p_Present->InitializeAddress, &IniChar, &(p_Present->PciDelgen), 4);
				p_Present->DelayGeneratorState.DT.ListLength = p_dt->ListLength;
				LargeInhibit = 0x0;
				i1 = 0;
				while ((LargeInhibit == 0x0) && (i1 < p_dt->ListLength))
				{
					LargeInhibit |= (p_dt->InhibitTime[i1] & 0xFC000000);
					i1++;
				}
				for (i1 = 0; i1 < p_dt->ListLength; i1++)
				{
					p_Present->DelayGeneratorState.DT.InhibitTime[i1] = p_dt->InhibitTime[i1];
					p_Present->DelayGeneratorState.DT.StepBack[i1] = p_dt->StepBack[i1];
					HighWord = 0x0;
					if (p_dt->InhibitTime[i1] > 0)
					{
						LowWord = (p_dt->InhibitTime[i1] - 1);
						HighWord |= 0x4;
					}
					else
						LowWord = 0x0;
					...
					result = Data32Out(p_Present->SecondaryCounterAddress, &HighWord, &(p_Present->PciDelgen), 4);
					result = Data32Out(p_Present->SecondaryCounterAddress, &LowWord, &(p_Present->PciDelgen), 4);
				}
				LoadChar = 0x40000000;
				*p_LoadWord |= LoadChar;
			}
	...
	default:
		if ((p_dt->InternalTrigger) || (p_dt->InternalArm))
		{
			if (p_dt->RepCounter != p_Present->DelayGeneratorState.DT.RepCounter) 
			{
				...
				result = DataOut(p_Present->ModuloCounterAddress, &(p_dt->RepCounter), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
...
long Set_Reference_channel(DelayChannel* p_rf, DG_InternalData* p_Present)
{
	long result;
	...
	case BME_DP01:
		if ((p_rf->GoSignal & 0x77) != (p_Present->DelayGeneratorState.T0.GoSignal & 0x77))
		{
			...
			{
				Value = 0x3A;
				result = DataOut(p_Present->Delay_cntrl, &Value, &(p_Present->PciDelgen), -1);
				if (result != DgSuccess)
					return result;
				if (DG_Product == BME_DP01)
				{
					...
					{
						Value = 0x30;
						result = DataOut(p_Present->Large_cntrl, &Value, &(p_Present->PciDelgen), -1);
						if (result != DgSuccess)
							return result;
						dummy = 0xFEFF;
						result = DataOut(p_Present->Large_T0, &dummy, &(p_Present->PciDelgen), 2);
						if (result != DgSuccess)
							return result;
					}
					else
		...
		if ((p_rf->GoSignal & 0x77) != 0x0)
		{
			result = DataOut(p_Present->Delay_T0, &(p_rf->Digital), &(p_Present->PciDelgen), 2);
			if (result != DgSuccess)
				return result;
			if (DG_Product == BME_DP01)
			{
				...
				if (p_rf->Digital_HW != 0)
				{
					result = DataOut(p_Present->Large_T0, &(p_rf->Digital_HW), &(p_Present->PciDelgen), 2);
					if (result != DgSuccess)
						return result;
				}
				else
	...
	case BME_SG02P5:
		if ((p_rf->GoSignal & 0x77) != 0x0)
		{
			...
			{
				p_Present->DelayGeneratorState.T0.Digital = p_rf->Digital;
				result = DataOut(p_Present->Delay_T0, &(p_rf->Digital), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
	...
	case BME_G03P2:
		if ((p_rf->GoSignal & 0x77) != 0x0)
		{
						...
						(p_rf->DigOff != p_Present->DelayGeneratorState.T0.DigOff))
			{
				...
				if (result != DgSuccess)
					return result;
				result = DataOut(p_Present->Delay_T0, &(p_rf->DigOff), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
	...
	case BME_SG05P3:
		if ((p_rf->GoSignal & 0x77) != 0x0)
		{
						...
						(p_rf->OutputModulo != p_Present->DelayGeneratorState.T0.OutputModulo))
			{
				...
				if (result != DgSuccess)
					return result;
				result = DataOut(p_Present->Delay_T0, &(p_rf->OutputModulo), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
	...
	case BME_SG05P3:
		if ((p_rf->GoSignal & 0x77) != 0x0)
		{
			if ((p_rf->Analog & 0x0FFF) != p_Present->DelayGeneratorState.T0.Analog)
			{
				...
				result = DataOut(p_Present->DAC_T0, &(InvertedValue), &(p_Present->PciDelgen), 2);
				if (result != DgSuccess)
					return result;
			}
		}
...
long Set_Delay_channel(DelayChannel* p_k, DelayChannel* p_ChState, short int pos, 
																															DG_InternalData* p_Present)
{
	long result;
	unsigned char* pch;
	...
	case BME_DP01:
		if ((p_k->GoSignal & 0x77) != (p_ChState->GoSignal & 0x77))
		{
			...
			{
				cntrl = 0x3A + (pos * 0x40);
				result = DataOut(p_Present->Delay_cntrl, &cntrl, &(p_Present->PciDelgen), -1);
				if (result != DgSuccess)
					return result;
				if (DG_Product == BME_DP01)
				{
					...
					{
						cntrl &= 0xF0;
						result = DataOut(p_Present->Large_cntrl, &cntrl, &(p_Present->PciDelgen), -1);
						if (result != DgSuccess)
							return result;
						dummy = 0xFEFF;
						result = DataOut((unsigned short)(p_Present->Large + pos), &dummy, &(p_Present->PciDelgen), 2);
						if (result != DgSuccess)
							return result;
					}
					else
		...
		if ((p_k->GoSignal & 0x77) != 0x0)
		{
			result = DataOut(DelayAddress, &(p_k->Digital), &(p_Present->PciDelgen), 2);
			if (result != DgSuccess)
				return result;
			if (DG_Product == BME_DP01)
			{
				...
				if (p_k->Digital_HW != 0)
				{
					result = DataOut((unsigned short)(p_Present->Large + pos), &(p_k->Digital_HW), 
			...
			result = DataOut(adr1, pch, &(p_Present->PciDelgen), -1);
			if (result != DgSuccess)
				return result;
			adr1 += 1;
			result = DataOut(adr1, (pch + 1), &(p_Present->PciDelgen), -1);
			if (result != DgSuccess)
				return result;
		}
		break;
	...
	case BME_SG02P5:
		if ((p_k->GoSignal & 0x77) != 0x0)
		{
			...
			{
				p_ChState->Digital = p_k->Digital;
				result = DataOut(DelayAddress, &(p_k->Digital), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
	...
	case BME_G03P2:
		if ((p_k->GoSignal & 0x77) != 0x0)
		{
						...
						(p_k->DigOff != p_ChState->DigOff))
			{
				...
				if (result != DgSuccess)
					return result;
				result = DataOut(DelayAddress, &(p_k->DigOff), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
	...
	case BME_SG05P3:
		if ((p_k->GoSignal & 0x77) != 0x0)
		{
						...
						(p_k->OutputModulo != p_ChState->OutputModulo))
			{
				...
				if (result != DgSuccess)
					return result;
				result = DataOut(DelayAddress, &(p_k->OutputModulo), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
		}
	...
	if ((p_k->GoSignal & 0x77) != 0x0)
	{
		if ((p_k->Analog & 0x0FFF) != p_ChState->Analog)
		{
			...
			case BME_SG05P3:
				...
				result = DataOut(DacAddress, &(InvertedValue), &(p_Present->PciDelgen), 2);
				if (result != DgSuccess)
					return result;
				break;
			default:
...
long Set_Delay_channel_SG05P4(DelayChannel* p_k, DelayChannel* p_ChState, unsigned long* p_LoadWord,
	...
	if ((p_k->GoSignal & 0x77) != 0x0)
	{
		...
		if(!same)
		{
			...
			for (i1 = 0; i1 < p_k->ListLength; i1++)
			{
				...
				result = Data32Out(DelayAddress, &(p_k->DelayList[i1]), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
				result = Data32Out(DelayAddress, &(p_k->PulseWidth[i1]), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
				result = Data32Out(DelayAddress, &(p_k->DacList[i1]), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			}
			*p_LoadWord |= LoadChar;
					...
					(p_k->OutputOffset != p_ChState->OutputOffset))
		{
			...
			result = Data32Out(ModuloAddress, &(p_k->OutputModulo), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			result = Data32Out(ModuloAddress, &(p_k->OutputOffset), &(p_Present->PciDelgen), 4);
				if (result != DgSuccess)
					return result;
			*p_LoadWord |= 0x20;
		}
...
long PrepareCounterControl(DG_BME_Registers* p_pdg, unsigned long* p_LoadWord, DG_InternalData* p_Present)
{
	long result;
	unsigned long DG_Product = p_Present->DG_Product;
	...
	case BME_SG08P4:
		...
		if ((p_Present->CommandRegisterState & 0x3F0011) != (p_Present->CommandRegister & 0x3F0011))
			*p_LoadWord |= 0x8;
		if (!p_Present->b_DG_Activated)
			result = LoadCounterControl(cctrl, p_Present);
		break;
	default:
...
void ModifyControlRegister(DG_BME_Registers* p_pdg, DG_InternalData* p_Present)
{
	...
	case BME_G05P2:
	case BME_G05P3:
		if ((p_pdg->DT.MS_Bus & 0x3) != (p_Present->DelayGeneratorState.DT.MS_Bus & 0x3))
		{
			...
			p_Present->DelayGeneratorState.DT.MS_Bus = (p_pdg->DT.MS_Bus & 0x3);
		}
		break;
	case BME_SG05P1:
	case BME_SG05P2:
	...
	case BME_SG08P3:
		...
		else
		{
			...
			for (i1 = 0; i1 < 5; i1++)
			{
				if ((p_Present->CommandRegister & TestChar) != 0x0)
					NoBusLines++;
				TestChar <<= 1;
			}
			if (NoBusLines != 4)
		...
		break;
	case BME_SG08P4:
		if ((p_pdg->DT.MS_Bus & 0x4F2) != (p_Present->DelayGeneratorState.DT.MS_Bus & 0x4F2))
		{
			NoBusLines = 0;
			if ((p_pdg->DT.MS_Bus & LocalSecondary) != 0x0)
			{
			...
			if ((p_pdg->DT.MS_Bus & StepBackOnBus) != 0x0)
			{
				...
				p_Present->CommandRegister &= ~0x2000000;
			if ((p_pdg->DT.MS_Bus & EnableOnBus) != 0x0)
			{
				p_Present->CommandRegister |= 0x4000000;
				NoBusLines++;
			}
			...
			if ((p_pdg->DT.MS_Bus & LoadDataOnBus) != 0x0)
			{
				p_Present->CommandRegister |= 0x20000000;
				NoBusLines++;
			}
			else
				p_Present->CommandRegister &= ~0x20000000;
			if (NoBusLines == 4)
			{
				p_Present->DelayGeneratorState.DT.MS_Bus |= (p_pdg->DT.MS_Bus & 0x4F2);
				p_Present->DelayGeneratorState.DT.MS_Bus &= (p_pdg->DT.MS_Bus | ~0x4F2);
			}
			else
			{
				p_Present->CommandRegister |= 0x3C000000;
				p_Present->CommandRegister &= ~0x3000000;
				p_Present->DelayGeneratorState.DT.MS_Bus |= 0x4E0;
				p_Present->DelayGeneratorState.DT.MS_Bus &= ~0x12;
			}
		}
		else
		{
	...
	if (p_pdg->DT.TriggerEnable != p_Present->DelayGeneratorState.DT.TriggerEnable)
	{
		...
		else
		{
			if ((DG_Product == SRS_DG135) || (DG_Product == BME_DP01))
				...
				p_Present->CommandRegister &= 0x1F3F3EEF;
		}
	...
	case BME_SG08P4:
		...
		for (i1 = 0; i1 < p_a->ListLength; i1++)
		{
			if (p_a->StepBack[i1] != p_b->StepBack[i1])
				return FALSE;
			if (p_a->InhibitTime[i1] != p_b->InhibitTime[i1])
				return FALSE;
		}
	case BME_SG08P1:
	case BME_SG08P2:
...
BOOL IsEqualPS(DG_BME_Registers* p_a, DG_BME_Registers* p_b, unsigned long DG_Product)
{
	...
	{
	case BME_SG08P4:
		if ((p_a->E.GoSignal & DirectGate) != (p_b->E.GoSignal & DirectGate))
			return FALSE;
		if ((p_a->F.GoSignal & DirectGate) != (p_b->F.GoSignal & DirectGate))
			return FALSE;
		if ((p_a->DT.MS_Bus & EnableFromE) != (p_b->DT.MS_Bus & EnableFromE))
			return FALSE;
		if ((p_a->DT.MS_Bus & DirectGate) != (p_b->DT.MS_Bus & DirectGate))
			return FALSE;
	case BME_SG08P3:
		if ((p_a->DT.MS_Bus & ListStepDelay) != (p_b->DT.MS_Bus & ListStepDelay))
			return FALSE;
		if (p_a->DT.UseF != p_b->DT.UseF)
			return FALSE;
	case BME_SG08P2:
		if ((p_a->DT.MS_Bus & ExtSynchFromF) != (p_b->DT.MS_Bus & ExtSynchFromF))
			return FALSE;
...
long Set_Prescaler(DG_BME_Registers* p_pdg, unsigned long* p_LoadWord, DG_InternalData* p_Present)
{
	long result;
	unsigned long Value;
	...
	case BME_SG08P3:
		...
		break;
	case BME_SG08P4:
		if ((p_pdg->DT.MS_Bus & EnableFromE) != (p_state->DT.MS_Bus & EnableFromE))
		{
			p_state->DT.MS_Bus |= (p_pdg->DT.MS_Bus & EnableFromE);
			p_state->DT.MS_Bus &= (p_pdg->DT.MS_Bus | ~EnableFromE);
			if ((p_pdg->DT.MS_Bus & EnableFromE) != 0x0)
			{
				pscl |= 0x800;
				if ((p_pdg->DT.MS_Bus & DirectGate) != (p_state->DT.MS_Bus & DirectGate))
				{
					p_state->DT.MS_Bus |= (p_pdg->DT.MS_Bus & DirectGate);
					p_state->DT.MS_Bus &= (p_pdg->DT.MS_Bus | ~DirectGate);
					if ((p_pdg->DT.MS_Bus & DirectGate) != 0x0)
						psch |= 0x20000000;
					else
						psch &= ~0x20000000;
				}
	...
	case BME_SG08P4:
		...
		{
			p_state->DT.TriggerMultiplier &= ~0x70F;
			pscl &= ~0x700;
			psct &= ~0xF;
		}
		if ((p_pdg->DT.ClockSource == CrystalOscillator) ||
		...
		{
			if (p_pdg->DT.ClockSource == TriggerAndOscillator)
				pscbits = (p_pdg->DT.OscillatorDivider / p_pdg->DT.TriggerDivider) + 7;
			else
				pscbits = 7;
		...
		if ((p_pdg->E.GoSignal & DirectGate) != (p_state->E.GoSignal & DirectGate))
		{
			p_state->E.GoSignal |= (p_pdg->E.GoSignal & DirectGate);
			p_state->E.GoSignal &= (p_pdg->E.GoSignal | ~DirectGate);
			if ((p_pdg->E.GoSignal & DirectGate) != 0x0)
				psch |= 0x40000000;
			else
				psch &= ~0x40000000;
		}
		...
		if ((p_pdg->F.GoSignal & DirectGate) != (p_state->F.GoSignal & DirectGate))
		{
			p_state->F.GoSignal |= (p_pdg->F.GoSignal & DirectGate);
			p_state->F.GoSignal &= (p_pdg->F.GoSignal | ~DirectGate);
			if ((p_pdg->F.GoSignal & DirectGate) != 0x0)
				psch |= 0x80000000;
			else
				psch &= ~0x80000000;
		}
		...
		if ((p_pdg->DT.MS_Bus & ListStepDelay) != (p_state->DT.MS_Bus & ListStepDelay))
		{
			p_state->DT.MS_Bus |= (p_pdg->DT.MS_Bus & ListStepDelay);
			p_state->DT.MS_Bus &= (p_pdg->DT.MS_Bus | ~ListStepDelay);
			if ((p_pdg->DT.MS_Bus & ListStepDelay) != 0x0)
				psch |= 0x10000000;
			else
				psch &= ~0x10000000;
		}
		...
		if ((p_pdg->DT.MS_Bus & ExtSynchFromF) != (p_state->DT.MS_Bus & ExtSynchFromF))
		{
			p_state->DT.MS_Bus |= (p_pdg->DT.MS_Bus & ExtSynchFromF);
			p_state->DT.MS_Bus &= (p_pdg->DT.MS_Bus | ~ExtSynchFromF);
			if ((p_pdg->DT.MS_Bus & ExtSynchFromF) != 0x0)
				psct |= 0x4000;
			else
				psct &= ~0x4000;
		}
		...
		if (res1 != NULL)
		{
			...
			switch (p_Present->DG_Product)
			{
				...
				case BME_SG08P2:
					...
					break;
				case BME_SG08P3:
					if (_stricmp(word, "BME_SG08P3"))
						return TRUE;
					break;
				case BME_SG08P4:
					if (_stricmp(word, "BME_SG08P4"))
						return TRUE;
					break;
				default:
					break;
...
void ResetDelayTrigger(DelayTriggerType* p_dt)
{
	...
	for (i1 = 0; i1 < MemDepth; i1++)
	{
		p_dt->InhibitTime[i1] = 0;
		p_dt->StepBack[i1] = i1;
	}
	p_dt->ListLength = 0;	
	p_dt->TriggerSignal = 0x0;
...
void CopyTriggerControl_V5(DelayTriggerData* p_dt, DelayTriggerData_V5* p_dtv5)
{
	ResetTriggerControl(p_dt);
	p_dt->InternalClock = p_dtv5->InternalClock;
	p_dt->InhibitSecondary = p_dtv5->InhibitSecondary;
	p_dt->ForceTrigger = p_dtv5->ForceTrigger;
	p_dt->StepBackTime = p_dtv5->StepBackTime;
	p_dt->GateDelay = p_dtv5->GateDelay;
	p_dt->BurstCounter = p_dtv5->BurstCounter;
	p_dt->TriggerLevel = p_dtv5->TriggerLevel;
	p_dt->GateLevel = p_dtv5->GateLevel;
	p_dt->DelaySecondary = p_dtv5->DelaySecondary;
	p_dt->PresetLong = p_dtv5->PresetLong;
	p_dt->MS_Bus = p_dtv5->MS_Bus;

	p_dt->OscillatorDivider = p_dtv5->OscillatorDivider;
	p_dt->TriggerDivider = p_dtv5->TriggerDivider;
	p_dt->TriggerMultiplier = p_dtv5->TriggerMultiplier;
	p_dt->ClockSource = p_dtv5->ClockSource;
	p_dt->GateDivider = p_dtv5->GateDivider;
	p_dt->PositiveGate = p_dtv5->PositiveGate;
	p_dt->IgnoreGate = p_dtv5->IgnoreGate;
	p_dt->SynchronizeGate = p_dtv5->SynchronizeGate;
	p_dt->ClockEnable = p_dtv5->ClockEnable;
	p_dt->InternalTrigger = p_dtv5->InternalTrigger;
	p_dt->InternalArm = p_dtv5->InternalArm;
	p_dt->SoftwareTrigger = p_dtv5->SoftwareTrigger;
	p_dt->RisingEdge = p_dtv5->RisingEdge;
	p_dt->StopOnPreset = p_dtv5->StopOnPreset;
	p_dt->ResetWhenDone = p_dtv5->ResetWhenDone;
	p_dt->TriggerEnable = p_dtv5->TriggerEnable;
	p_dt->Terminate = p_dtv5->Terminate;
	p_dt->GateTerminate = p_dtv5->GateTerminate;
	p_dt->UseF = p_dtv5->UseF;
}
...
long SetTrigger(DG_BME_Registers* p_dgr, DG_BME_Control* p_BDG, long DG_Number)
{
	...
	int i1,t_count,InhibitZeroShift,ForceZeroShift;
	p_Present = (p_Internal + DG_Number);
	DG_Product = p_Present->DG_Product;
	...
	if (p_dgr->DT.SoftwareTrigger)
		t_count++;
	if (t_count > 1)
		return IncompatibleTriggerModes;
	...
	switch (p_dgr->DG_Product)
	{
		case BME_SG08P3:
		case BME_SG08P4:
			if (p_BDG->DT.ListLength > 0)
			{
				p_dgr->DT.TriggerSignal = p_BDG->DT.TriggerSignal;
				p_dgr->DT.ListLength = p_BDG->DT.ListLength;
				for (i1 = 0; i1 < p_BDG->DT.ListLength; i1++)
				{
					p_dgr->DT.InhibitTime[i1] = max(0, (RoundInt((p_BDG->DT.InhibitTime[i1] / Increment)) - InhibitZeroShift));
					p_dgr->DT.StepBack[i1] = p_BDG->DT.StepBack[i1];
				}
			}
		case BME_SG08P1:
		case BME_SG08P2:
			...
			if (((p_dgr->DT.MS_Bus & ResynchMultiply) != 0x0) && ((p_dgr->DT.TriggerMultiplier & 0x7) != 0x0))
				p_dgr->DT.TriggerMultiplier |= 0x8;
			else
				p_dgr->DT.TriggerMultiplier &= ~0x8;
...
long SetReference(DelayChannel* p_a, DelayChannelData* p_dl, 
															unsigned long MainCounter, unsigned long OutputLevel, long DG_Number)
{
	double Remainder;
	DG_InternalData* p_Present;
	...
	if ((p_dl->Terminate) && (p_dl->HighDrive) && (OutputLevel == TTL_VoltageLevel))
		return InvalidOutputCombination;
	p_a->Terminate = p_dl->Terminate;
	p_a->HighDrive = p_dl->HighDrive;
	if ((p_a->GoSignal & 0x77) != 0x0)
	{
		if (p_dl->FireFirst < 0.0)
			return DelayTimeNegative;
		switch (DG_Product)
		{
		case BME_G03V3:
		case BME_G03V4:
...
{
	DG_InternalData* p_Present;
	double value,Delta,DeltaMax,Increment;
	...
	*p_digital = 0;
	...
	{
		Increment = p_Present->TimeIncrement;
		DeltaMax = 0.1 * 4095.0 / 4096.0;
		Delta = min(DeltaMax, max(0.02, Increment));
		if (p_Present->CalibrationLevel == CalibrateOffset)
		{
		...
		if (((p_Present->DG_Product == BME_SG08P1) || (p_Present->DG_Product == BME_SG08P2) || (p_Present->DG_Product == BME_SG08P3) || (p_Present->DG_Product == BME_SG08P4))
				&& ((InValue - ZeroTime) <= Delta))
		...
		else
		{
			...
			else
			{
				...
				case BME_G03V1:
				case BME_SG02V1:
					*p_remainder = max(0.0, min(DeltaMax, (InValue - p1Time)));
					*p_digital = 1;
					break;
				default:
					if (InValue >= p1Time)
					{
						if ((p2Time - InValue) < (InValue - (p1Time + DeltaMax)))
							*p_digital = 2;
						else
						{
							*p_remainder = min(DeltaMax, (InValue - p1Time));
							*p_digital = 1;
						}
					}
					else
					{
						if ((p1Time - InValue) < (InValue - (ZeroTime + DeltaMax)))
							*p_digital = 1;
						else
							*p_remainder = min(DeltaMax, max(0.0, (InValue - ZeroTime)));
					}
				}
			}
		}
