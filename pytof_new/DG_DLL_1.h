...
//
// long IgnoreCommands(BOOL Ignore);
//
//		call this function if you want to write information to the state variable but
//		not to the FPGA registers of the delay generator. This function may need to be
//		called, if you want to regain control of the delay generator after a computer crash.
//
//					Ignore:			Set this parameter to TRUE, if you want to inhibit writing to the
//											delay generator FPGA registers
//
...
//
//long ReadModuleCalibration(double* p_InternalOscillatorFrequency, double* p_ExternalOscillatorFrequency, 
//																		double* p_TenMhzp2Time, double* p_TenMhzp1Time, double* p_TenMhzZeroTime, double* p_TenMhzResetTime, 
//																		int* p_DG_Product, int* p_MS_Module, long DG_Number);
//
//		this function returns all the calibration constants of one delay generator module
//
//					p_InternalOscillatorFrequency:    Internal Oscillator Freuency
//					p_ExternalOscillatorFrequency:    External Oscillator Freuency
//					p_TenMhzp2Time:                   Time Shift after second Clock Period scaled to 10MHz Oscillator Feqeuency
//					p_TenMhzp1Time:                   Time Shift after first Clock Period scaled to 10MHz Oscillator Feqeuency
//					p_TenMhzZeroTime:                 Direct Trigger Time Shift scaled to 10MHz Oscillator Feqeuency
//					p_DG_Product:                     product Number of Delay Generator
//					p_MS_Module:                      1 for master module, 2 for slave module
//					DG_Number:	                      number of this delay generator (counting starts from zero!)
//
//
...
//													function SetDelayGenerator
//					p_Control_V5:  	structure of type DG_BME_Control_V5 that is passed to the
//													function SetDelayGenerator
//
...
//					bit 3 TRUE: it is necessary to call the function JumpStartAllLists(...)
...
_declspec (dllexport) long CC Set_TriggerParameters(BOOL TriggerTerminate, double InternalClock, 
													...
													long DG_Number);
//
// long Set_ResetWhenDone(BOOL ResetWhenDone, long DG_Number);
//
//		call this function with the parameter ResetWhenDone = FALSE before deactivating
//    the delay generator to deactivate it in a synchronuous fashion;
//
...
//
// long Set_G08_UnsynchParameters(DG_BME_Control* p_BDG, long DG_Number);
//
//		call these functions to set parameters that must not be transferred synchronuously
//		to the delay generator. 
//
...
//
//
// long Set_InhibitList(double* p_InhibitTime, unsigned long* p_StepBack, signed long ListLength, 
//																										unsigned long TriggerSignal, long DG_Number);
//
//		call this function to load a list of inhibit values into the FPGA of the delay generator
//
//					p_InhibitTime:		pointer to an array of double containing a list of inhibit times for the main 
//														trigger in µsec, after which the trigger circuit will be reactivated.
//					p_StepBack:				pointer to an array of unsigned long containing a list of stepback 
//														addresses if the stepback counter elapses.
//					ListLength:				length of the lists to be passed.
//					TriggerSignal:		the bits of this unsigned long define which of the signals start, stepback, 
//														etc. controlles the sequence of the InhibitTime list. These signals can 
//														or’ed. The corresponding constants are defined in ‘’DG_Data.h’’.)
//					DG_Number:				number of this delay generator (counting starts from zero!)
//								
...
//
// long CalibrateGroupDirect(int* p_module, double* p_internal, double* p_external)
//
//		calibrate the complete set of delay generators, using also the information
//		which ones of these are slaves of defined masters, also explicitly passing
//		the internal and external frequencies of the individual delay generator cards
//
//					p_module:		integer vector as long as the number of modules defined
//											in the routine ''Reserve_DG_Data(...)''.Write the constant
//											''MasterModule'' onto the vector for a master, write
//											''Slavemodule'' for a slave. (The master should always come
//											before its slaves.)
//					p_internal:	double vector as long as the number of modules defined
//											in the routine ''Reserve_DG_Data(...)''.Write onto each
//											position of this vector the respective frequency of its 
//											internal oscillator in MHz.
//					p_external:	double vector as long as the number of modules defined
//											in the routine ''Reserve_DG_Data(...)''.Write onto each
//											position of this vector the respective frequency of its 
//											external oscillator frequency entered at the trigger 
//											input connector.
//
...
typedef struct 
{
  ...
  HANDLE h_DelayGenerator;
	...
	long Set_Delay_channel(DelayChannel* p_k, DelayChannel* p_m, short int pos, DG_InternalData* p_Present);
	long Set_Delay_channel_SG05P4(DelayChannel* p_k, DelayChannel* p_ChState, unsigned long* LoadWord,
